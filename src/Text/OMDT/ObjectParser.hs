{-# OPTIONS_GHC -fno-warn-overlapping-patterns #-}
module Text.OMDT.ObjectParser
    ( omdObject, OMDScalar(..)
    ) where
import qualified Text.OMDT.Lexer as Lex

import Data.List
import Data.Object (Object(..))
import Data.Version (Version)

-- parser produced by Happy Version 1.18.4

data HappyAbsSyn t4 t5 t6 t7 t8 t9 t10 t11 t12 t13 t14
	= HappyTerminal (Lex.Token)
	| HappyErrorToken Int
	| HappyAbsSyn4 t4
	| HappyAbsSyn5 t5
	| HappyAbsSyn6 t6
	| HappyAbsSyn7 t7
	| HappyAbsSyn8 t8
	| HappyAbsSyn9 t9
	| HappyAbsSyn10 t10
	| HappyAbsSyn11 t11
	| HappyAbsSyn12 t12
	| HappyAbsSyn13 t13
	| HappyAbsSyn14 t14

action_0 (15) = happyShift action_7
action_0 (17) = happyShift action_11
action_0 (20) = happyShift action_12
action_0 (21) = happyShift action_13
action_0 (22) = happyShift action_14
action_0 (23) = happyShift action_15
action_0 (24) = happyShift action_16
action_0 (25) = happyShift action_17
action_0 (26) = happyShift action_18
action_0 (27) = happyShift action_19
action_0 (4) = happyGoto action_8
action_0 (5) = happyGoto action_9
action_0 (6) = happyGoto action_10
action_0 (7) = happyGoto action_2
action_0 (9) = happyGoto action_3
action_0 (11) = happyGoto action_4
action_0 (12) = happyGoto action_5
action_0 (14) = happyGoto action_6
action_0 _ = happyReduce_16

action_1 (15) = happyShift action_7
action_1 (7) = happyGoto action_2
action_1 (9) = happyGoto action_3
action_1 (11) = happyGoto action_4
action_1 (12) = happyGoto action_5
action_1 (14) = happyGoto action_6
action_1 _ = happyFail

action_2 _ = happyReduce_1

action_3 _ = happyReduce_24

action_4 _ = happyReduce_15

action_5 _ = happyReduce_18

action_6 (15) = happyShift action_7
action_6 (9) = happyGoto action_27
action_6 (12) = happyGoto action_5
action_6 _ = happyReduce_20

action_7 (25) = happyShift action_26
action_7 (10) = happyGoto action_25
action_7 _ = happyFail

action_8 (15) = happyShift action_7
action_8 (17) = happyShift action_11
action_8 (20) = happyShift action_12
action_8 (21) = happyShift action_13
action_8 (22) = happyShift action_14
action_8 (23) = happyShift action_15
action_8 (24) = happyShift action_16
action_8 (25) = happyShift action_17
action_8 (26) = happyShift action_18
action_8 (27) = happyShift action_19
action_8 (28) = happyAccept
action_8 (4) = happyGoto action_22
action_8 (5) = happyGoto action_9
action_8 (6) = happyGoto action_10
action_8 (7) = happyGoto action_2
action_8 (8) = happyGoto action_23
action_8 (9) = happyGoto action_3
action_8 (11) = happyGoto action_4
action_8 (12) = happyGoto action_5
action_8 (13) = happyGoto action_24
action_8 (14) = happyGoto action_6
action_8 _ = happyFail

action_9 _ = happyReduce_2

action_10 _ = happyReduce_5

action_11 (26) = happyShift action_21
action_11 _ = happyFail

action_12 _ = happyReduce_11

action_13 _ = happyReduce_12

action_14 _ = happyReduce_13

action_15 _ = happyReduce_14

action_16 _ = happyReduce_10

action_17 _ = happyReduce_4

action_18 (19) = happyShift action_20
action_18 _ = happyReduce_6

action_19 _ = happyReduce_7

action_20 (26) = happyShift action_32
action_20 _ = happyFail

action_21 (18) = happyShift action_31
action_21 _ = happyFail

action_22 (15) = happyShift action_7
action_22 (16) = happyReduce_22
action_22 (17) = happyShift action_11
action_22 (20) = happyShift action_12
action_22 (21) = happyShift action_13
action_22 (22) = happyShift action_14
action_22 (23) = happyShift action_15
action_22 (24) = happyShift action_16
action_22 (25) = happyShift action_17
action_22 (26) = happyShift action_18
action_22 (27) = happyShift action_19
action_22 (28) = happyReduce_22
action_22 (4) = happyGoto action_22
action_22 (5) = happyGoto action_9
action_22 (6) = happyGoto action_10
action_22 (7) = happyGoto action_2
action_22 (8) = happyGoto action_23
action_22 (9) = happyGoto action_3
action_22 (11) = happyGoto action_4
action_22 (12) = happyGoto action_5
action_22 (13) = happyGoto action_24
action_22 (14) = happyGoto action_6
action_22 _ = happyReduce_22

action_23 _ = happyReduce_3

action_24 (15) = happyShift action_7
action_24 (16) = happyReduce_17
action_24 (17) = happyShift action_11
action_24 (20) = happyShift action_12
action_24 (21) = happyShift action_13
action_24 (22) = happyShift action_14
action_24 (23) = happyShift action_15
action_24 (24) = happyShift action_16
action_24 (25) = happyShift action_17
action_24 (26) = happyShift action_18
action_24 (27) = happyShift action_19
action_24 (28) = happyReduce_17
action_24 (4) = happyGoto action_30
action_24 (5) = happyGoto action_9
action_24 (6) = happyGoto action_10
action_24 (7) = happyGoto action_2
action_24 (9) = happyGoto action_3
action_24 (11) = happyGoto action_4
action_24 (12) = happyGoto action_5
action_24 (14) = happyGoto action_6
action_24 _ = happyReduce_17

action_25 (16) = happyShift action_29
action_25 _ = happyFail

action_26 (15) = happyShift action_7
action_26 (17) = happyShift action_11
action_26 (20) = happyShift action_12
action_26 (21) = happyShift action_13
action_26 (22) = happyShift action_14
action_26 (23) = happyShift action_15
action_26 (24) = happyShift action_16
action_26 (25) = happyShift action_17
action_26 (26) = happyShift action_18
action_26 (27) = happyShift action_19
action_26 (4) = happyGoto action_28
action_26 (5) = happyGoto action_9
action_26 (6) = happyGoto action_10
action_26 (7) = happyGoto action_2
action_26 (9) = happyGoto action_3
action_26 (11) = happyGoto action_4
action_26 (12) = happyGoto action_5
action_26 (14) = happyGoto action_6
action_26 _ = happyReduce_16

action_27 _ = happyReduce_25

action_28 (15) = happyShift action_7
action_28 (16) = happyReduce_19
action_28 (17) = happyShift action_11
action_28 (20) = happyShift action_12
action_28 (21) = happyShift action_13
action_28 (22) = happyShift action_14
action_28 (23) = happyShift action_15
action_28 (24) = happyShift action_16
action_28 (25) = happyShift action_17
action_28 (26) = happyShift action_18
action_28 (27) = happyShift action_19
action_28 (4) = happyGoto action_22
action_28 (5) = happyGoto action_9
action_28 (6) = happyGoto action_10
action_28 (7) = happyGoto action_2
action_28 (8) = happyGoto action_23
action_28 (9) = happyGoto action_3
action_28 (11) = happyGoto action_4
action_28 (12) = happyGoto action_5
action_28 (13) = happyGoto action_24
action_28 (14) = happyGoto action_6
action_28 _ = happyReduce_19

action_29 _ = happyReduce_21

action_30 (15) = happyShift action_7
action_30 (16) = happyReduce_23
action_30 (17) = happyShift action_11
action_30 (20) = happyShift action_12
action_30 (21) = happyShift action_13
action_30 (22) = happyShift action_14
action_30 (23) = happyShift action_15
action_30 (24) = happyShift action_16
action_30 (25) = happyShift action_17
action_30 (26) = happyShift action_18
action_30 (27) = happyShift action_19
action_30 (28) = happyReduce_23
action_30 (4) = happyGoto action_22
action_30 (5) = happyGoto action_9
action_30 (6) = happyGoto action_10
action_30 (7) = happyGoto action_2
action_30 (8) = happyGoto action_23
action_30 (9) = happyGoto action_3
action_30 (11) = happyGoto action_4
action_30 (12) = happyGoto action_5
action_30 (13) = happyGoto action_24
action_30 (14) = happyGoto action_6
action_30 _ = happyReduce_23

action_31 _ = happyReduce_9

action_32 (19) = happyShift action_33
action_32 _ = happyFail

action_33 (26) = happyShift action_34
action_33 _ = happyFail

action_34 _ = happyReduce_8

happyReduce_1 = happySpecReduce_1  4 happyReduction_1
happyReduction_1 (HappyAbsSyn7  happy_var_1)
	 =  HappyAbsSyn4
		 (Mapping happy_var_1
	)
happyReduction_1 _  = notHappyAtAll 

happyReduce_2 = happySpecReduce_1  4 happyReduction_2
happyReduction_2 (HappyAbsSyn5  happy_var_1)
	 =  HappyAbsSyn4
		 (Scalar  happy_var_1
	)
happyReduction_2 _  = notHappyAtAll 

happyReduce_3 = happySpecReduce_2  4 happyReduction_3
happyReduction_3 (HappyAbsSyn8  happy_var_2)
	(HappyAbsSyn4  happy_var_1)
	 =  HappyAbsSyn4
		 (Sequence (happy_var_1:happy_var_2)
	)
happyReduction_3 _ _  = notHappyAtAll 

happyReduce_4 = happySpecReduce_1  5 happyReduction_4
happyReduction_4 (HappyTerminal (Lex.String  _ happy_var_1))
	 =  HappyAbsSyn5
		 (String happy_var_1
	)
happyReduction_4 _  = notHappyAtAll 

happyReduce_5 = happySpecReduce_1  5 happyReduction_5
happyReduction_5 (HappyAbsSyn6  happy_var_1)
	 =  HappyAbsSyn5
		 (Bool   happy_var_1
	)
happyReduction_5 _  = notHappyAtAll 

happyReduce_6 = happySpecReduce_1  5 happyReduction_6
happyReduction_6 (HappyTerminal (Lex.Int   _ _ happy_var_1))
	 =  HappyAbsSyn5
		 (Int    happy_var_1
	)
happyReduction_6 _  = notHappyAtAll 

happyReduce_7 = happySpecReduce_1  5 happyReduction_7
happyReduction_7 (HappyTerminal (Lex.Frac  _ _ happy_var_1))
	 =  HappyAbsSyn5
		 (Frac   happy_var_1
	)
happyReduction_7 _  = notHappyAtAll 

happyReduce_8 = happyReduce 5 5 happyReduction_8
happyReduction_8 ((HappyTerminal (Lex.Int   _ _ happy_var_5)) `HappyStk`
	_ `HappyStk`
	(HappyTerminal (Lex.Int   _ _ happy_var_3)) `HappyStk`
	_ `HappyStk`
	(HappyTerminal (Lex.Int   _ _ happy_var_1)) `HappyStk`
	happyRest)
	 = HappyAbsSyn5
		 (Date happy_var_1 happy_var_3 happy_var_5
	) `HappyStk` happyRest

happyReduce_9 = happySpecReduce_3  5 happyReduction_9
happyReduction_9 _
	(HappyTerminal (Lex.Int   _ _ happy_var_2))
	_
	 =  HappyAbsSyn5
		 (Footnote happy_var_2
	)
happyReduction_9 _ _ _  = notHappyAtAll 

happyReduce_10 = happySpecReduce_1  5 happyReduction_10
happyReduction_10 (HappyTerminal (Lex.Version _ happy_var_1))
	 =  HappyAbsSyn5
		 (Version happy_var_1
	)
happyReduction_10 _  = notHappyAtAll 

happyReduce_11 = happySpecReduce_1  6 happyReduction_11
happyReduction_11 _
	 =  HappyAbsSyn6
		 (True
	)

happyReduce_12 = happySpecReduce_1  6 happyReduction_12
happyReduction_12 _
	 =  HappyAbsSyn6
		 (True
	)

happyReduce_13 = happySpecReduce_1  6 happyReduction_13
happyReduction_13 _
	 =  HappyAbsSyn6
		 (False
	)

happyReduce_14 = happySpecReduce_1  6 happyReduction_14
happyReduction_14 _
	 =  HappyAbsSyn6
		 (False
	)

happyReduce_15 = happySpecReduce_1  7 happyReduction_15
happyReduction_15 (HappyAbsSyn11  happy_var_1)
	 =  HappyAbsSyn7
		 (happy_var_1
	)
happyReduction_15 _  = notHappyAtAll 

happyReduce_16 = happySpecReduce_0  7 happyReduction_16
happyReduction_16  =  HappyAbsSyn7
		 ([]
	)

happyReduce_17 = happySpecReduce_1  8 happyReduction_17
happyReduction_17 (HappyAbsSyn13  happy_var_1)
	 =  HappyAbsSyn8
		 (reverse happy_var_1
	)
happyReduction_17 _  = notHappyAtAll 

happyReduce_18 = happySpecReduce_1  9 happyReduction_18
happyReduction_18 (HappyAbsSyn12  happy_var_1)
	 =  HappyAbsSyn9
		 (happy_var_1
	)
happyReduction_18 _  = notHappyAtAll 

happyReduce_19 = happySpecReduce_2  10 happyReduction_19
happyReduction_19 (HappyAbsSyn4  happy_var_2)
	(HappyTerminal (Lex.String  _ happy_var_1))
	 =  HappyAbsSyn10
		 ((happy_var_1,happy_var_2)
	)
happyReduction_19 _ _  = notHappyAtAll 

happyReduce_20 = happySpecReduce_1  11 happyReduction_20
happyReduction_20 (HappyAbsSyn14  happy_var_1)
	 =  HappyAbsSyn11
		 (reverse happy_var_1
	)
happyReduction_20 _  = notHappyAtAll 

happyReduce_21 = happySpecReduce_3  12 happyReduction_21
happyReduction_21 _
	(HappyAbsSyn10  happy_var_2)
	_
	 =  HappyAbsSyn12
		 (happy_var_2
	)
happyReduction_21 _ _ _  = notHappyAtAll 

happyReduce_22 = happySpecReduce_1  13 happyReduction_22
happyReduction_22 (HappyAbsSyn4  happy_var_1)
	 =  HappyAbsSyn13
		 ([happy_var_1]
	)
happyReduction_22 _  = notHappyAtAll 

happyReduce_23 = happySpecReduce_2  13 happyReduction_23
happyReduction_23 (HappyAbsSyn4  happy_var_2)
	(HappyAbsSyn13  happy_var_1)
	 =  HappyAbsSyn13
		 (happy_var_2 : happy_var_1
	)
happyReduction_23 _ _  = notHappyAtAll 

happyReduce_24 = happySpecReduce_1  14 happyReduction_24
happyReduction_24 (HappyAbsSyn9  happy_var_1)
	 =  HappyAbsSyn14
		 ([happy_var_1]
	)
happyReduction_24 _  = notHappyAtAll 

happyReduce_25 = happySpecReduce_2  14 happyReduction_25
happyReduction_25 (HappyAbsSyn9  happy_var_2)
	(HappyAbsSyn14  happy_var_1)
	 =  HappyAbsSyn14
		 (happy_var_2 : happy_var_1
	)
happyReduction_25 _ _  = notHappyAtAll 

happyNewToken action sts stk [] =
	action 28 28 notHappyAtAll (HappyState action) sts stk []

happyNewToken action sts stk (tk:tks) =
	let cont i = action i i tk (HappyState action) sts stk tks in
	case tk of {
	Lex.OpenParen      _ -> cont 15;
	Lex.CloseParen     _ -> cont 16;
	Lex.OpenBracket    _ -> cont 17;
	Lex.CloseBracket   _ -> cont 18;
	Lex.Slash          _ -> cont 19;
	Lex.String _ happy_dollar_dollar@"TRUE" -> cont 20;
	Lex.String _ happy_dollar_dollar@"Yes" -> cont 21;
	Lex.String _ happy_dollar_dollar@"FALSE" -> cont 22;
	Lex.String _ happy_dollar_dollar@"No" -> cont 23;
	Lex.Version _ happy_dollar_dollar -> cont 24;
	Lex.String  _ happy_dollar_dollar -> cont 25;
	Lex.Int   _ _ happy_dollar_dollar -> cont 26;
	Lex.Frac  _ _ happy_dollar_dollar -> cont 27;
	_ -> happyError' (tk:tks)
	}

happyError_ tk tks = happyError' (tk:tks)

newtype HappyIdentity a = HappyIdentity a
happyIdentity = HappyIdentity
happyRunIdentity (HappyIdentity a) = a

instance Monad HappyIdentity where
    return = HappyIdentity
    (HappyIdentity p) >>= q = q p

happyThen :: () => HappyIdentity a -> (a -> HappyIdentity b) -> HappyIdentity b
happyThen = (>>=)
happyReturn :: () => a -> HappyIdentity a
happyReturn = (return)
happyThen1 m k tks = (>>=) m (\a -> k a tks)
happyReturn1 :: () => a -> b -> HappyIdentity a
happyReturn1 = \a tks -> (return) a
happyError' :: () => [(Lex.Token)] -> HappyIdentity a
happyError' = HappyIdentity . parseError

omdObject tks = happyRunIdentity happySomeParser where
  happySomeParser = happyThen (happyParse action_0 tks) (\x -> case x of {HappyAbsSyn4 z -> happyReturn z; _other -> notHappyAtAll })

happySeq = happyDontSeq


data OMDScalar
    = String    String
    | Bool      Bool
    | Int       Integer
    | Frac      Rational
    | Date      Integer Integer Integer
    | Footnote  Integer
    | Version   Version
    deriving (Eq, Show)

parseError tokens = error ("Parse error near " ++ show (head tokens))
{-# LINE 1 "templates/GenericTemplate.hs" #-}
{-# LINE 1 "templates/GenericTemplate.hs" #-}
{-# LINE 1 "<built-in>" #-}
{-# LINE 1 "<command-line>" #-}
{-# LINE 1 "templates/GenericTemplate.hs" #-}
-- Id: GenericTemplate.hs,v 1.26 2005/01/14 14:47:22 simonmar Exp 

{-# LINE 28 "templates/GenericTemplate.hs" #-}








{-# LINE 49 "templates/GenericTemplate.hs" #-}

{-# LINE 59 "templates/GenericTemplate.hs" #-}

{-# LINE 68 "templates/GenericTemplate.hs" #-}

infixr 9 `HappyStk`
data HappyStk a = HappyStk a (HappyStk a)

-----------------------------------------------------------------------------
-- starting the parse

happyParse start_state = happyNewToken start_state notHappyAtAll notHappyAtAll

-----------------------------------------------------------------------------
-- Accepting the parse

-- If the current token is (1), it means we've just accepted a partial
-- parse (a %partial parser).  We must ignore the saved token on the top of
-- the stack in this case.
happyAccept (1) tk st sts (_ `HappyStk` ans `HappyStk` _) =
	happyReturn1 ans
happyAccept j tk st sts (HappyStk ans _) = 
	 (happyReturn1 ans)

-----------------------------------------------------------------------------
-- Arrays only: do the next action

{-# LINE 155 "templates/GenericTemplate.hs" #-}

-----------------------------------------------------------------------------
-- HappyState data type (not arrays)



newtype HappyState b c = HappyState
        (Int ->                    -- token number
         Int ->                    -- token number (yes, again)
         b ->                           -- token semantic value
         HappyState b c ->              -- current state
         [HappyState b c] ->            -- state stack
         c)



-----------------------------------------------------------------------------
-- Shifting a token

happyShift new_state (1) tk st sts stk@(x `HappyStk` _) =
     let i = (case x of { HappyErrorToken (i) -> i }) in
--     trace "shifting the error token" $
     new_state i i tk (HappyState (new_state)) ((st):(sts)) (stk)

happyShift new_state i tk st sts stk =
     happyNewToken new_state ((st):(sts)) ((HappyTerminal (tk))`HappyStk`stk)

-- happyReduce is specialised for the common cases.

happySpecReduce_0 i fn (1) tk st sts stk
     = happyFail (1) tk st sts stk
happySpecReduce_0 nt fn j tk st@((HappyState (action))) sts stk
     = action nt j tk st ((st):(sts)) (fn `HappyStk` stk)

happySpecReduce_1 i fn (1) tk st sts stk
     = happyFail (1) tk st sts stk
happySpecReduce_1 nt fn j tk _ sts@(((st@(HappyState (action))):(_))) (v1`HappyStk`stk')
     = let r = fn v1 in
       happySeq r (action nt j tk st sts (r `HappyStk` stk'))

happySpecReduce_2 i fn (1) tk st sts stk
     = happyFail (1) tk st sts stk
happySpecReduce_2 nt fn j tk _ ((_):(sts@(((st@(HappyState (action))):(_))))) (v1`HappyStk`v2`HappyStk`stk')
     = let r = fn v1 v2 in
       happySeq r (action nt j tk st sts (r `HappyStk` stk'))

happySpecReduce_3 i fn (1) tk st sts stk
     = happyFail (1) tk st sts stk
happySpecReduce_3 nt fn j tk _ ((_):(((_):(sts@(((st@(HappyState (action))):(_))))))) (v1`HappyStk`v2`HappyStk`v3`HappyStk`stk')
     = let r = fn v1 v2 v3 in
       happySeq r (action nt j tk st sts (r `HappyStk` stk'))

happyReduce k i fn (1) tk st sts stk
     = happyFail (1) tk st sts stk
happyReduce k nt fn j tk st sts stk
     = case happyDrop (k - ((1) :: Int)) sts of
	 sts1@(((st1@(HappyState (action))):(_))) ->
        	let r = fn stk in  -- it doesn't hurt to always seq here...
       		happyDoSeq r (action nt j tk st1 sts1 r)

happyMonadReduce k nt fn (1) tk st sts stk
     = happyFail (1) tk st sts stk
happyMonadReduce k nt fn j tk st sts stk =
        happyThen1 (fn stk tk) (\r -> action nt j tk st1 sts1 (r `HappyStk` drop_stk))
       where sts1@(((st1@(HappyState (action))):(_))) = happyDrop k ((st):(sts))
             drop_stk = happyDropStk k stk

happyMonad2Reduce k nt fn (1) tk st sts stk
     = happyFail (1) tk st sts stk
happyMonad2Reduce k nt fn j tk st sts stk =
       happyThen1 (fn stk tk) (\r -> happyNewToken new_state sts1 (r `HappyStk` drop_stk))
       where sts1@(((st1@(HappyState (action))):(_))) = happyDrop k ((st):(sts))
             drop_stk = happyDropStk k stk





             new_state = action


happyDrop (0) l = l
happyDrop n ((_):(t)) = happyDrop (n - ((1) :: Int)) t

happyDropStk (0) l = l
happyDropStk n (x `HappyStk` xs) = happyDropStk (n - ((1)::Int)) xs

-----------------------------------------------------------------------------
-- Moving to a new state after a reduction

{-# LINE 253 "templates/GenericTemplate.hs" #-}
happyGoto action j tk st = action j j tk (HappyState action)


-----------------------------------------------------------------------------
-- Error recovery ((1) is the error token)

-- parse error if we are in recovery and we fail again
happyFail  (1) tk old_st _ stk =
--	trace "failing" $ 
    	happyError_ tk

{-  We don't need state discarding for our restricted implementation of
    "error".  In fact, it can cause some bogus parses, so I've disabled it
    for now --SDM

-- discard a state
happyFail  (1) tk old_st (((HappyState (action))):(sts)) 
						(saved_tok `HappyStk` _ `HappyStk` stk) =
--	trace ("discarding state, depth " ++ show (length stk))  $
	action (1) (1) tk (HappyState (action)) sts ((saved_tok`HappyStk`stk))
-}

-- Enter error recovery: generate an error token,
--                       save the old token and carry on.
happyFail  i tk (HappyState (action)) sts stk =
--      trace "entering error recovery" $
	action (1) (1) tk (HappyState (action)) sts ( (HappyErrorToken (i)) `HappyStk` stk)

-- Internal happy errors:

notHappyAtAll = error "Internal Happy error\n"

-----------------------------------------------------------------------------
-- Hack to get the typechecker to accept our action functions







-----------------------------------------------------------------------------
-- Seq-ing.  If the --strict flag is given, then Happy emits 
--	happySeq = happyDoSeq
-- otherwise it emits
-- 	happySeq = happyDontSeq

happyDoSeq, happyDontSeq :: a -> b -> b
happyDoSeq   a b = a `seq` b
happyDontSeq a b = b

-----------------------------------------------------------------------------
-- Don't inline any functions from the template.  GHC has a nasty habit
-- of deciding to inline happyGoto everywhere, which increases the size of
-- the generated parser quite a bit.

{-# LINE 317 "templates/GenericTemplate.hs" #-}
{-# NOINLINE happyShift #-}
{-# NOINLINE happySpecReduce_0 #-}
{-# NOINLINE happySpecReduce_1 #-}
{-# NOINLINE happySpecReduce_2 #-}
{-# NOINLINE happySpecReduce_3 #-}
{-# NOINLINE happyReduce #-}
{-# NOINLINE happyMonadReduce #-}
{-# NOINLINE happyGoto #-}
{-# NOINLINE happyFail #-}

-- end of Happy Template.
